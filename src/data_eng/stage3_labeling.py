...
class FeatureEngineering:
    ...
    def data_(self, config: DataEngConfig):
        try:
            ...
            logging.info("Exception occurred while loading the data" + str(e))
            logging.info("Failed to load the data please check your code and run")
            raise AppException(e, sys) from e

    # outlier detection
    def outlier_detection(self, data: pd.DataFrame, colname: str) -> pd.DataFrame:
        self.data = data[data[colname] <= (data[colname].mean() + 3 * data[colname].std())]
        return self.data

    def remove_outliers(self, config: DataEngConfig) -> pd.DataFrame:
        try:
            ...
            logging.info("removed outliers function compiled successfully")
            return self.data
        except Exception as e:
            logging.info("Exception occured in remove_outliers method" + str(e))
    ...
    def freight_cost_transform(self, config: DataEngConfig) -> pd.DataFrame:
        try:
            logging.info("'freight_cost_transform' FUNCTION STARTED")
    ...
    def feature_engineering(self, config: DataEngConfig) -> pd.DataFrame:
        try:
            ...
            self.data["first_line_designation"] = pd.get_dummies(
                self.data["first_line_designation"], drop_first=True, dtype=int
            )

            self.data["shipment_mode"] = lb.fit_transform(self.data["shipment_mode"])
            logging.info("feature engineering function compiled successfully")
            return self.data
    ...
    def final_data(self, config: DataEngConfig):  # input_path, output_path):
        try:
            logging.info("'data' FUNCTION STARTED")
            self.finaldata = self.feature_engineering(config)
            self.data.drop("Unnamed: 0", axis=1, inplace=True)
            self.data.to_csv(
                config.processed_data.processed_data_dir + "/" + config.processed_data.processed_filename, index=False
            )
