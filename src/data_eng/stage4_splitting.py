...
class SplitData:
    ...
    def split_data(self, config: DataEngConfig):
        logging.info("Starting data splitting...")

        # Apply feature engineering (assumes it returns a transformed DataFrame)
        self.data = pd.read_csv(
            config.processed_data.processed_data_dir + "/" + config.processed_data.processed_filename, sep=","
        )
        logging.info("Feature engineering retrieved.")

        # Split data
        self.train, self.test = train_test_split(
            self.data, test_size=config.splited_data.split_ratio, random_state=config.splited_data.random_state
        )
        logging.info("Data split into train and test sets.")

        # Save files locally (basic names for now)
        self.train.to_csv(
            config.splited_data.train_data_dir + "/" + config.splited_data.train_filename,
            sep=",",
            index=False,
            encoding="UTF-8",
        )
        self.test.to_csv(
            config.splited_data.test_data_dir + "/" + config.splited_data.test_filename,
            sep=",",
            index=False,
            encoding="UTF-8",
        )
